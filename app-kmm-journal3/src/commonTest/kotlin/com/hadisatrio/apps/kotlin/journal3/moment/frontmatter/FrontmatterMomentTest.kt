...
class FrontmatterMomentTest {
    ...
    private val moments = FrontmatterMoments(fileSystem, "content/moments".toPath(), memorables)
    ...
    @Test
    fun `Writes detail updates to the filesystem`() {
        val moment = moments.new()
        ...
        val path = "content/moments/${moment.id}.md".toPath()
    ...
    @Test
    fun `Writes attachment updates to the filesystem`() {
        val moment = moments.new()
        val arbitraryExternalFilePath = "foo".toPath().apply {
            fileSystem.sink(this).buffer().use { it.writeUtf8("bar") }
        }
