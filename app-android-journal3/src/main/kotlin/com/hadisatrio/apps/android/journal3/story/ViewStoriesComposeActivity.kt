...
import com.hadisatrio.libs.android.compose.StringsListComposable
...
class ViewStoriesComposeActivity : AppCompatActivity() {
    ...
    private val scaffoldComposable by lazy {
        JournalScaffoldComposable(
            fabEventFactory = { SelectionEvent("action", "add_story") },
            content = { listComposable() }
        )
    }
    private val listComposable by lazy {
        StringsListComposable(
            clickEventFactory = { SelectionEvent("item_position", it.toString()) },
        )
    }
